...
module.exports = {
    ...
    login: (req, res) => {
        ...
        getEmployeeByUserName(body.usc_name, (err, results) => {
            ...
            const db_password = results[0].usc_pass.toString();

            if (get_password === db_password) {
                results.usc_pass = undefined;
                const jsontoken = sign({ result: results }, "@dhj$&$(*)dndkm76$%jdn(^$6GH%^ahy484&*((73kk))lko65HSJ58*&%gd75hfgJdk237^5jKIj8&6)kBG*&F^%V*-", {
                    expiresIn: "12h"
                });
                return res.status(200).json({
                    success: 1,
